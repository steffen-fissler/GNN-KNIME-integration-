...
class GNN(torch.nn.Module):
    def __init__(self,num_of_feat, hidden_layer, num_class, convolutional_layer):
        super(GNN, self).__init__()
        if convolutional_layer == "GCNConv":
            self.conv1 = GCNConv(num_of_feat, hidden_layer)
            self.conv2 = GCNConv(hidden_layer, num_class)

        if convolutional_layer == "SAGEConv":
            self.conv1 = SAGEConv(num_of_feat, hidden_layer)
            self.conv2 = SAGEConv(hidden_layer, num_class)
