...
@knext.node(
    name="GNN Graph Creator",
    node_type=knext.NodeType.MANIPULATOR,
    icon_path="icons/icon.png",
    category=utils.category,
)
@knext.input_table(
    name="Nodes with Features",
...
class GNNCreator:
    ...
    key = knext.ColumnParameter(
        label="key", description="Unique id of each row", port_index=1
    )
    target = knext.ColumnParameter(
        label="target", description="The label (y) of each row", port_index=1
    )
    edge_list_01 = knext.ColumnParameter(
        label="edge_list_01", description="The first column of edges", port_index=0
    )
    edge_list_02 = knext.ColumnParameter(
        label="edge_list_02", description="The second column of edges", port_index=0
    )
    ...
    def execute(self, exec_context, input_1, input_2):
        ...
        g = self.construct_graph(nodes_data=nodes_data, edges_data=edges_data)

        # Pass graph, saved num_of_class, and Nodes ID column
        graph_dict = {"graph": g, "num_of_class": num_class, "key": self.key}
    ...
    def construct_graph(self, nodes_data, edges_data):
        ...
        node_features_list = nodes_data.drop(
            columns=[self.key, self.target]
        ).values.tolist()
...
class GNNLearner:
    ...
    def train(self, model, g, exec_context):
        ...
        for e in range(epochs + 1):
            ...
            loss = self.masked_loss(predictions=out, labels=g.y, mask=g.data_mask)
            ...
            train_accuracy = self.masked_accuracy(
                predictions=out, labels=g.y, mask=g.data_mask
            )
...
@knext.input_table(
    name="Test Set", description="A table that contains nodes for testing."
)
@knext.output_table(
...
class GNNPredictor:
    ...
    def execute(self, exec_context, model_object, test_set):
        ...
        if self.prediction_confidence:
            ...
            output_table = pa.table(
                [pa.array(key), pa.array(labels), pa.array(predictions)],
